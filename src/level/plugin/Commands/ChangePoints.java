...
public class ChangePoints extends BasicLevelsCommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if (cmd.getName().equalsIgnoreCase("changepoints")) {
            Player player = getSenderPlayer(sender);
            if (args.length < 2) {
                sender.sendMessage(Messages.getMessage(player, "ChangePointsUsage"));
            } else {
                ...
                try {
                    Main.getPlayerData(player_to_string).setPoints(number);
